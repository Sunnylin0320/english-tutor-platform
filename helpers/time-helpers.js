...
module.exports = {
  ...
  // 計算出學生可選擇的時段
  getAvailablePeriod: (classData, scheduledClassesArr) => {
    ...
    // 刪除被預約時段
    for (let i = 0; i < scheduledClassesArr.length; i++) {
      const reservedDate = dayjs(scheduledClassesArr[i].classTime).format(
        'YYYY-MM-DD'
      )

      const reservedTime = dayjs(scheduledClassesArr[i].classTime).format(
        'HH:mm'
      )

      const duration = scheduledClassesArr[i].spendTime
      ...
      availablePeriod[date].forEach((classTime, i) => {
        if (
          ...
          (classData.spendTime === 60 &&
            dayjs(classTime, 'HH:mm').add(30, 'minute').format('HH:mm') ===
              availablePeriod[date][i + 1])
        ) {
          const endTime = dayjs(classTime, 'HH:mm')
            .add(classData.spendTime, 'minute')
            .format('HH:mm')
          resultArr.push(`${date} ${classTime} to ${endTime}`)
        }
      })
    }
